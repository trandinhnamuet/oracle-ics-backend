...
async function exportAdminKey() {
  ...
  if (!encryptionSecret) {
    ...
    console.log('   Fallback: ***REMOVED-ENC-KEY***');
  }
  ...
  try {
    ...
    console.log(`   Using encryption secret: ${encryptionSecret || '***REMOVED-ENC-KEY***'}`);
    ...
    try {
      ...
      console.error('3. If unsure, try setting it to: "***REMOVED-ENC-KEY***"');
