...
export function generateSSHKeyPair(bits: number = 4096): SSHKeyPair {
  logger.log(`Generating SSH key pair (${bits} bits)`);
  ...
  // NOTE: do not log the public key, private key, or fingerprint - they are
  // sensitive and (especially the fingerprint) can correlate sessions across logs.
  logger.log('SSH key pair generated successfully');
...
export function saveKeyPairToFile(keyPair: SSHKeyPair, keyName: string = 'admin'): void {
  ...
  logger.log(`Admin SSH keys saved to ${keyDir}`);
}
...
export function loadKeyPairFromFile(keyName: string = 'admin'): SSHKeyPair | null {
  ...
  if (!fs.existsSync(publicKeyPath) || !fs.existsSync(privateKeyPath)) {
    logger.warn(`Key files not found in ${keyDir}`);
    return null;
  }
  ...
  if (publicKeyFromFile.startsWith('ssh-rsa ') && isValidOpenSSH) {
    ...
    logger.log('Public key in PEM format, converting to OpenSSH');
    try {
      publicKey = convertToOpenSSHFormat(publicKeyFromFile);
      // Update file with correct format
      fs.writeFileSync(publicKeyPath, publicKey, { mode: 0o644 });
      logger.log('Converted and updated public key file to OpenSSH format');
    } catch (error) {
      logger.error(`Failed to convert public key: ${error.message}`);
      return null;
    }
  }

  const fingerprint = calculateFingerprint(publicKey);
  // Do not log the fingerprint or any portion of the key material.
  logger.log('Loaded admin SSH key pair from file');
