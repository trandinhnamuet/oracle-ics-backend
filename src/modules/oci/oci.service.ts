...
`;
      ...
      if (!isWindowsImage && cloudInitConfig) {
        ...
        const windowsSetupScript = [
          ...
          '# 1. Install OpenSSH Server — try WindowsCapability (Win 2019+), fallback to manual install (Win 2016)',
          '$sshdInstalled = $false',
          'try {',
          '  $cap = Get-WindowsCapability -Online -ErrorAction Stop | Where-Object { $_.Name -like "OpenSSH.Server*" }',
          '  if ($cap) {',
          '    if ($cap.State -eq "Installed") { $sshdInstalled = $true }',
          '    else { Add-WindowsCapability -Online -Name $cap.Name -ErrorAction Stop; $sshdInstalled = $true }',
          '  }',
          '} catch {',
          '  # Get-WindowsCapability not available (Windows Server 2016) — install OpenSSH manually',
          '  try {',
          '    $sshdExe = "$env:SystemRoot\\System32\\OpenSSH\\sshd.exe"',
          '    if (-not (Test-Path $sshdExe)) {',
          '      # Download Win32-OpenSSH release from GitHub (latest v9.5 supports Server 2016)',
          ...
          '      # Run install-sshd.ps1 to register the sshd service',
          '      & "$destDir\\install-sshd.ps1" 2>$null',
          '      # Add to PATH so ssh-keygen, sshd etc. are found',
          ...
          '    if (-not (Test-Path $kgen)) { $kgen = "$env:ProgramFiles\\OpenSSH\\ssh-keygen.exe" }',
          '    if (Test-Path $kgen) { & $kgen -A 2>$null } else { ssh-keygen -A 2>$null }',
          '  }',
          '} catch {}',
          ...
          '  # Win 2016 manual install: copy default config if sshd_config does not exist yet',
          '  if (-not (Test-Path $sshdCfgPath)) {',
          '    $defaultCfg = "$env:ProgramFiles\\OpenSSH\\sshd_config_default"',
          '    if (Test-Path $defaultCfg) {',
          '      if (-not (Test-Path "$env:ProgramData\\ssh")) { New-Item -ItemType Directory -Path "$env:ProgramData\\ssh" -Force | Out-Null }',
          '      Copy-Item $defaultCfg $sshdCfgPath -Force',
          '    }',
          '  }',
          ...
          `    $cfg = $cfg -replace "(?m)^\\s*#\\s*(Match Group administrators)", '$1'`,
          `    $cfg = $cfg -replace "(?m)^\\s*#\\s*(\\s*AuthorizedKeysFile\\s+__PROGRAMDATA__)", '$1'`,
          ...
          '} catch {}',
          '',
          '# ===== Set default RDP login user to opc =====',
          '# Windows LogonUI caches the last-created account; this resets it to opc so that',
          '# RDP shows opc pre-selected instead of icsreset on first login.',
          'try {',
          '  Set-ItemProperty -Path "HKLM:\\SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Winlogon" -Name "DefaultUserName" -Value "opc" -Force -ErrorAction SilentlyContinue',
          ...
          '  Set-ItemProperty -Path "HKLM:\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Authentication\\LogonUI" -Name "LastLoggedOnUser" -Value ".\\opc" -Force -ErrorAction SilentlyContinue',
          '  Set-ItemProperty -Path "HKLM:\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Authentication\\LogonUI" -Name "LastLoggedOnSAMUser" -Value ".\\opc" -Force -ErrorAction SilentlyContinue',
          '  Set-ItemProperty -Path "HKLM:\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Authentication\\LogonUI" -Name "LastLoggedOnDisplayName" -Value "opc" -Force -ErrorAction SilentlyContinue',
          '} catch {}',
          '',
          '# Stop transcript log',
