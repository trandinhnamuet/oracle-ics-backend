...
`;
      ...
      if (!isWindowsImage && cloudInitConfig) {
        ...
        const windowsSetupScript = [
          ...
          '  }',
          '  # Hide icsreset from the Windows logon/RDP screen so users only see opc.',
          '  # SpecialAccounts\\UserList value=0 makes the account invisible to logon UI.',
          '  $saPath = "HKLM:\\SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Winlogon\\SpecialAccounts\\UserList"',
          '  if (-not (Test-Path $saPath)) { New-Item -Path $saPath -Force -ErrorAction SilentlyContinue | Out-Null }',
          '  Set-ItemProperty -Path $saPath -Name $adminUser -Value 0 -Type DWord -Force -ErrorAction SilentlyContinue',
          '} catch {}',
          '',
          '# Stop transcript log',
